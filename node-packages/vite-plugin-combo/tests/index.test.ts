...
describe("vite-plugin-combo", () => {
  it("handles missing configuration", () => {
    ...
    /* @ts-ignore */
    expect(() => combo({})).toThrowError('vite-plugin-combo: missing configuration for "input".')
  })
  ...
  it("configures default cors.origin values", () => {
    ...
    ;[
      ...
      expect(resolvedConfig.server.cors.origin.some((regex) => test(regex, url))).toBe(true),
    )
    ...
    ;[
      ...
      expect(resolvedConfig.server.cors.origin.some((regex) => test(regex, url))).toBe(false),
    )
  })
